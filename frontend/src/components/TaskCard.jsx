export default function TaskCard({ task, currentUser, canEdit = true, canDelete = true, onEdit, onDelete }) {
  const t = task;
    ...
    <div className={`task-card ${t.status}`}>
      ...
      <div className="task-meta">
        ...
        {t.assignedTo && typeof t.assignedTo === 'object' && (
          <span className="task-owner">→ {t.assignedTo.name}</span>
        )}
        {t.projectId && typeof t.projectId === 'object' && (
          <span className="task-owner">{t.projectId.name}</span>
        )}
      </div>
